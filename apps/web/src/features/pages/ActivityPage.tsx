...
import { Activity, AlertTriangle, CheckCircle2, Clock3, RefreshCw, Sparkles } from "lucide-react";
...
export function ActivityPage() {
  ...
  const skillEvents = useQuery({ queryKey: ["skill-evolution-events"], queryFn: chatApi.getSkillEvolutionEvents, refetchInterval: 3000 });
  ...
  return <PageScaffold title="后台活动" description="Cognitive Worker 与前台解耦；失败不会替换旧记忆。" icon={Activity} action={<button className="secondary-button" onClick={() => jobs.refetch()}><RefreshCw />手动刷新</button>}>
    ...
    <h2 className="section-heading">Skill 演化链路</h2>
    <div className="activity-table"><div className="table-head"><span>事件</span><span>Skill / Revision</span><span>时间</span><span>原因</span></div>{(skillEvents.data ?? []).map((event) => <div className="table-row" key={event.id}><span><Sparkles />{event.event_type.replace("skill.", "")}</span><span>{event.skill_id.slice(0, 8)} · {event.revision_id?.slice(0, 8) ?? "—"}</span><span>{new Date(event.created_at).toLocaleString()}</span><span>{event.reason ?? "确定性系统事件"}</span></div>)}{!skillEvents.isLoading && !skillEvents.data?.length ? <p className="empty-state">尚无 Skill 演化事件。</p> : null}</div>
  </PageScaffold>;
}
