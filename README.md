...
UTA Agent 是一个公开源码、实验性的本地优先 AI Agent，尝试让 AI 在长期交互中保存经验、理解用户偏好，并根据真实反馈调整后续行为。
...
UTA Agent 采用 React、FastAPI、SQLite、Tauri 等技术构建，支持 OpenAI 兼容模型接口、本地工具调用、结构化执行轨迹和 Windows 桌面运行。对话、工具执行、Token 账本、记忆版本和 Skill 版本均保存在本地。

它不声称已经实现真正的自主进化或 AGI，而是一次围绕记忆、反馈、工具、Skill 和版本管理的持续学习实验。

项目尚不成熟，维护者资源有限，目前已暂停持续维护。欢迎其他开发者协助维护、复现和验证，也欢迎提出不成熟但可讨论的想法、问题报告和改进建议。
...
这是一个采用 MIT 许可证的开源实验项目。维护者时间和资源有限，欢迎提交可复现的问题、测试改进、文档修正和小范围修复，也欢迎先通过 Issue 讨论不成熟的想法。提交 Issue 或 Pull Request 前请阅读 [`CONTRIBUTING.md`](CONTRIBUTING.md)；版权与使用边界见 [`LICENSE`](LICENSE)。
